...
def generarpcap(ip, miip, npaquetes=200):
    ...
    try:
        ...
        print "DESACTIVADO se va a reseteear porque el telnet mando eof sin login"
        #resetear(ip)
    ...
    # agrego mas para que solo sean paquetes http
    tn.write("./tcpdump -c %s -i br0 -s 0 -w - 'tcp port 80 and (((ip[2:2] - ((ip[0]&0xf)<<2)) - ((tcp[12]&0xf0)>>2)) != 0)'| ./busybox-mips gzip -9 > textfile.pcap.gz\n" % npaquetes)
    salidadump = ""
    tiempo = 0
    while "#" not in salidadump:
        salidadump += tn.read_until("nunca",1) #algo que no encuentra nunca, es solo para tener timeout
        tn.write(" \x08") # para que no crea que estamos inactivos (espacio y borrar)
        tiempo += 1
        #if tiempo >= 120:
        #    tn.write("\x03")
        #    break
    ...
    if "cannot execute" in salidadump:
        ...
        raise ValueError
    if "not found" in salidadump:
        print color.rojo + "Fallados los archivos tcpdump y busybox-mips, los borro" + color.fin
        tn.write("rm busybox-mips\n")
        tn.read_until("#")
        tn.write("rm tcpdump\n")
        tn.read_until("#")
        raise ValueError
